"""Connection lifecycle for New Relic Connector."""
from __future__ import annotations
import json, uuid
from imperal_sdk import ActionResult
from new_relic_client import NewRelicClient
...
@chat.function(
    "connect_new_relic",
    "Connect New Relic account via credentials.",
    action_type="write",
    chain_callable=True,
    event="new-relic-connector.connect_new_relic",
    effects=["create:connection"],
    data_model=ConnectParams
)
async def connect_new_relic(params: ConnectParams, ctx) -> ActionResult[ConnectionRecord]:
    """Connect New Relic Connector."""
    ...
    record = {
        "id": cid,
        "label": params.label or "New Relic Account",
...
@chat.function(
    "list_connections",
    "List connected New Relic accounts.",
    action_type="read",
    chain_callable=True,
    data_model=NoParams
)
...
@chat.function(
    "disconnect_new_relic",
    "Disconnect New Relic account.",
    action_type="write",
    chain_callable=True,
    event="new-relic-connector.disconnect_new_relic",
    effects=["delete:connection"],
    data_model=ConnectionIdParams
)
async def disconnect_new_relic(params: ConnectionIdParams, ctx) -> ActionResult[DeleteResult]:
    conns = await _load_connections(ctx)
